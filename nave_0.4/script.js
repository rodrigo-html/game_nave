...
var recordista = document.querySelector('#recordista')
var recordistaNome
var bombas
var pente
//Posição
...
btOk.addEventListener('click', ()=>{
    if(document.forms[0].elements[2].value=='selecione'||document.forms[0].elements[1].value==""){
        alert('selecione uma opção válida')
    }
    if(document.forms[0].elements[2].value=='facil'){
        recordistaNome = document.forms[0].elements[1].value
...
function start(vB,vN,tmp,dif){
    ...
    ct = dif
    rec.innerHTML=story
    recordista.innerHTML=localStorage.getItem(`ranking${ct}`)
        ...
        for(b=0;b<bombas.length;b++){
            for(t=0;t<pente.length;t++){
                ...
                if(tiroPy<bombaPy+bombas[b].offsetHeight&&tiroPx<bombaPx+bombas[b].offsetWidth&&tiroPx+pente[t].offsetWidth>bombaPx&&tiroPy+pente[t].offsetHeight>tiroPy){
                    ...
                    if(pontos>story){
                        localStorage.setItem(ct,pontos)
                        localStorage.setItem('ranking'+ct,recordistaNome)
                    }
                    res.value=pontos
